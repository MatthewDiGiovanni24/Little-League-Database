...
Application can be executed by running the little-league-schema.sql file and then the little-league-data.sql file in Oracle SQL Developer or any SQL program. Comment out the table drops if it is the first time the schema script is run. Then, the query file or other queries can be ran.
